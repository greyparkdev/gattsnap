...
///
/// Hand-rolled rather than taken from CryptoKit (Apple-only) or swift-crypto (a
/// dependency this package does not otherwise need) because Core must build on
/// Linux. Verified against the standard NIST vectors in the test suite.
public enum SHA256 {
    ...
    public static func digest(_ message: [UInt8]) -> [UInt8] {
        ...
        // Pad to a multiple of 64 bytes: 0x80, zeros, then the 64-bit bit count.
        var padded = message
